...
ulong Disasm(uchar const *cmd,ulong cmdsize,ulong ip,t_disasm *da,
  ...
  if (success==0) {
    ...
    if (pcmd==NULL) {
      if (damode & DA_DUMP)
        Thexdump(da->dump,(uchar*)cmd,1,config->lowercase);
      if (damode & DA_TEXT) {
        ...
        Thexdump(da->result+j,(uchar*)cmd,1,config->lowercase); };
      da->size=1;
      da->nprefix=0;
      da->prefixes=0;
      da->cmdtype=D_BAD;
  ...
  for (i=0; i<NOPERAND; i++) {
    ...
    switch (arg & B_ARGMASK) {
      ...
      case B_INT128:                   // 128-bit integer in ModRM, memory only
        k=Operandmodrm(&im,16,(uchar*)(cmd+n),cmdsize-n,op);
        if (k<0) break;                // Error in address
        if (k==0) {
          // Register is not allowed, decode as 32-bit register and set error.
          Operandintreg(&im,4,cmd[n] & 0x07,op);
      ...
      case B_IMMINT:                   // 8/16/32-bit int at immediate addr
        Operandimmaddr(&im,datasize,(uchar*)(cmd+n),cmdsize-n,op);
        im.usesdatasize=1;
        break;
      case B_INTPAIR:                  // Two signed 16/32 in ModRM, memory only
        k=Operandmodrm(&im,2*datasize,(uchar*)(cmd+n),cmdsize-n,op);
        if (k<0) break;                // Error in address
      ...
      case B_SEGOFFS:                  // 16:16/16:32 absolute address in memory
        k=Operandmodrm(&im,datasize+2,(uchar*)(cmd+n),cmdsize-n,op);
        if (k<0) break;                // Error in address
        if (k==0) {
      ...
      case B_LONGDATA:                 // Long data in ModRM, mem only
        k=Operandmodrm(&im,256,(uchar*)(cmd+n),cmdsize-n,op);
        if (k<0) break;                // Error in address
      ...
      case B_FLOAT80:                  // 80-bit float in ModRM, memory only
        k=Operandmodrm(&im,10,(uchar*)(cmd+n),cmdsize-n,op);
        if (k<0) break;                // Error in address
        if (k==0) {
          // Register is not allowed, decode as FPU register and set error.
          Operandfpureg(&im,cmd[n] & 0x07,op);
      ...
      case B_SSEF64x2:                 // SSE reg/memory as 2 64-bit floats
        k=Operandmodrm(&im,
          (arg & B_NOVEXSIZE?16:im.ssesize),(uchar*)(cmd+n),cmdsize-n,op);
        if (k<0) break;                // Error in address
      ...
      case B_SSEF32x2L:                // Low 2 32-bit floats in SSE reg/memory
        k=Operandmodrm(&im,
          (arg & B_NOVEXSIZE?16:im.ssesize)/2,(uchar*)(cmd+n),cmdsize-n,op);
        if (k<0) break;                // Error in address
        if (k==0)                      // Operand in SSE register
          Operandssereg(&im,cmd[n] & 0x07,op);
        op->granularity=4;
      ...
      case B_OFFSET:                   // 16/32 const offset from next command
        Operandoffset(&im,datasize,datasize,(uchar*)(cmd+n),cmdsize-n,da->ip+n,op);
        break;
      case B_BYTEOFFS:                 // 8-bit sxt const offset from next cmd
        Operandoffset(&im,1,datasize,(uchar*)(cmd+n),cmdsize-n,da->ip+n,op);
        break;
      case B_FARCONST:                 // 16:16/16:32 absolute address constant
        Operandimmfaraddr(&im,datasize,(uchar*)(cmd+n),cmdsize-n,op);
        break;
      case B_DESCR:                    // 16:32 descriptor in ModRM
        k=Operandmodrm(&im,6,(uchar*)(cmd+n),cmdsize-n,op);
        if (k<0) break;                // Error in address
        if (k==0) {
          // Register is not allowed, decode as 32-bit register and set error.
          Operandintreg(&im,4,cmd[n] & 0x07,op);
      ...
      case B_CONST8:                   // Immediate 8-bit constant
        Operandimmconst(&im,1,1,datasize,(uchar*)(cmd+n),cmdsize-n,0,op);
      ...
      case B_CONST8_2:                 // Immediate 8-bit const, second in cmd
        Operandimmconst(&im,1,1,datasize,(uchar*)(cmd+n),cmdsize-n,1,op);
        break;
      case B_CONST16:                  // Immediate 16-bit constant
        Operandimmconst(&im,2,2,datasize,(uchar*)(cmd+n),cmdsize-n,0,op);
        break;
      case B_CONST:                    // Immediate 8/16/32-bit constant
      case B_CONSTL:                   // Immediate 16/32-bit constant
        Operandimmconst(&im,datasize,datasize,datasize,(uchar*)(cmd+n),cmdsize-n,0,op);
        im.usesdatasize=1;
        break;
      case B_SXTCONST:                 // Immediate 8-bit sign-extended to size
        Operandimmconst(&im,1,datasize,datasize,(uchar*)(cmd+n),cmdsize-n,0,op);
        im.usesdatasize=1;
        break;
...
error:
  ...
  if (damode & DA_DUMP) {
    if (da->errors & DAE_CROSS)        // Incomplete command
      Thexdump(da->dump,(uchar*)cmd,cmdsize,config->lowercase);
    else {
      j=0;
      ...
      for (u=0; u<da->nprefix; u++) {
        j+=Thexdump(da->dump+j,(uchar*)(cmd+u),1,config->lowercase);
      ...
      j+=Thexdump(da->dump+j,(uchar*)(cmd+u),im.mainsize+im.modsize-u,
        config->lowercase);
      // Dump displacement, if any, separated with space from command's body.
      if (im.dispsize>0) {
        da->dump[j++]=T(' ');
        j+=Thexdump(da->dump+j,(uchar*)cmd+im.mainsize+im.modsize,im.dispsize,
          config->lowercase);
        ;
      };
      ...
      if (im.immsize2>0) {
        da->dump[j++]=T(' ');
        Thexdump(da->dump+j,(uchar*)cmd+im.mainsize+im.modsize+im.dispsize+im.immsize1,
